<template>
  <div class="w-full flex justify-center">
    <div
      id="weather_forecast_card"
      class="card min-w-sm max-w-sm border opacity-50 transition-shadow test shadow-lg hover:shadow-shadow-xl w-full rounded-md"
    >
      <div class="rounded-lg py-6 px-8 w-full bg-gray-900 text-white h-full">
        <button
          type="button"
          class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
        >
          <FontAwesomeIcon :icon="['fa', 'close']" class="fa-2x" />
          <span class="sr-only">Close modal</span>
        </button>
        <div class="mt-5">
          <h2 class="font-bold text-3xl leading-none pb-1">{{ weekDay }}</h2>
          <h3 class="leading-none pb-2 pl-1">{{ dateTitle }}</h3>
        </div>
        <template
          v-if="!props.asyncLoading && !openWeatherLoadingErrorMsg.length"
        >
          <div class="mt-5 mb-10">
            <p class="flex aling-center opacity-75">
              {{
                props.weatherForecastViewData.locationFormatText +
                ",  " +
                localTime
              }}
            </p>
          </div>
          <div class="space-y-2">
            <span class="flex space-x-2 items-center">
              <FontAwesomeIcon :icon="['fas', 'droplet']" />
              <span>{{ props.weatherForecastViewData.humidity }}%</span></span
            >
            <span class="flex space-x-2 items-center"
              ><FontAwesomeIcon :icon="['fas', 'wind']" />
              <span
                >{{ props.weatherForecastViewData.windSpeed }} meter/sec</span
              ></span
            >
            <span class="flex space-x-2 items-center"
              ><FontAwesomeIcon :icon="['fas', 'temperature-full']" />
              <span
                >{{ props.weatherForecastViewData.pressure }} hPa</span
              ></span
            >
            <span class="flex space-x-2 items-center"
              ><FontAwesomeIcon :icon="['fas', 'binoculars']" />
              <span
                >{{ props.weatherForecastViewData.visibility }} meter</span
              ></span
            >
          </div>
          <div>
            <img :src="weatherIcon" width="300" height="150" />

            <div class="flex">
              <div class="w-1/2">
                <strong class="leading-none text-5xl block font-weight-bolder"
                  >{{ props.weatherForecastViewData.tempCelsius }}ºC</strong
                >
                <b class="text-2xl block font-bold">{{
                  props.weatherForecastViewData.weatherDescription
                }}</b>
              </div>
              <div class="w-1/2 pt-2 pl-10">
                <h2 class="leading-none pb-2">Min temp</h2>
                <h3 class="font-bold text-3xl leading-none">
                  {{ props.weatherForecastViewData.tempMinCelsius }}ºC
                </h3>
                <h2 class="leading-none pt-4 pb-2">Max temp</h2>
                <h3 class="font-bold text-3xl leading-none pb-2">
                  {{ props.weatherForecastViewData.tempMaxCelsius }}ºC
                </h3>
              </div>
            </div>
          </div>
        </template>
        <div
          class="flex justify-center"
          v-else-if="props.asyncLoading"
          role="status"
        >
          <svg
            aria-hidden="true"
            class="w-48 h-48 pt-16 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
            viewBox="0 0 100 101"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
              fill="currentColor"
            />
            <path
              d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
              fill="currentFill"
            />
          </svg>
          <span class="sr-only">Loading...</span>
        </div>
        <div
          class="flex mt-20 space-x-2 items-center"
          v-if="openWeatherLoadingErrorMsg.length > 0"
        >
          <div class="flex min-w-0 gap-x-4">
            <FontAwesomeIcon
              :icon="['fas', 'bug']"
              class="fa-2x"
              style="color: red"
            />
          </div>
          <div class="min-w-0 flex-auto px-4">
            <span class="font-bold text-2xl text-red-400">{{
              openWeatherLoadingErrorMsg
            }}</span>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup>
import { FontAwesomeIcon } from "@fortawesome/vue-fontawesome";
import { computed, defineProps } from "vue";
import WeatherComposition from "../types/WeatherComposition";

const props = defineProps({
  weatherForecastViewData: Object,
  openWeatherLoadingErrorMsg: String,
  asyncLoading: Boolean,
});

const weekDay = computed(() => {
  return new Intl.DateTimeFormat("en-US", { weekday: "long" }).format(
    new Date()
  );
});
const dateTitle = computed(() => {
  return new Intl.DateTimeFormat("en-GB", {
    day: "numeric",
    month: "long",
    year: "numeric",
  }).format(new Date());
});

const localTime = computed(() => {
  const today = new Date();
  const localDateTime = new Date(
    today.getUTCFullYear(),
    today.getUTCMonth(),
    today.getUTCDate(),
    today.getUTCHours(),
    today.getUTCMinutes(),
    today.getUTCSeconds()
  );
  localDateTime.setHours(
    localDateTime.getHours() + props.weatherForecastViewData.timezone / 3600
  );
  let localHoursFmt =
    localDateTime.getHours() < 10
      ? "0".concat(localDateTime.getHours())
      : localDateTime.getHours();
  let localMinutesFmt =
    localDateTime.getMinutes() < 10
      ? "0".concat(localDateTime.getMinutes())
      : localDateTime.getMinutes();

  return localHoursFmt + ":" + localMinutesFmt + " (local time)";
});

const weatherIcon = computed(() => {
  const g =
    WeatherComposition.WeatherConditions[
      props?.weatherForecastViewData.weatherParameterGroup
    ];
  if (g) {
    return g.findWeatherAsset(props.weatherForecastViewData.id);
  }
  return "../assets/indicators/cloud.png";
});
</script>

<style scoped>
#weather_forecast_card {
  width: 450px;
  height: 100vh;
}
</style>

